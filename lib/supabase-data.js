...
export async function loadAllData() {
  ...
  const [
    ...
    supabase.from('pos_sales').select('*').eq('company_id', companyId).order('created_at'),
    supabase.from('payables').select('*').eq('company_id', companyId).order('created_at'),
  ]);
  ...
  return {
    ...
    pos: (posSales || []).map(s => ({
      id: s.id, date: s.sale_date, ts: s.ts || '', items: s.items || [], sub: Number(s.subtotal), tax: Number(s.tax), total: Number(s.total),
      method: s.method, received: Number(s.received), change: Number(s.change_amount), st: s.status, settled: s.settled, settledDate: s.settled_date || null, cid: s.customer_id || null, staff: s.staff || '',
    })),
    payables: (payables || []).map(p => ({
      id: p.id, supplier: p.supplier, date: p.payable_date, due: p.due_date, amt: Number(p.amount), tax: Number(p.tax), total: Number(p.total),
      paid: Number(p.paid), st: p.status, paidDate: p.paid_date || null, desc: p.description || '', cat: p.account || '仕入高',
    })),
...
// ===== POS売上 =====
export async function dbAddPosSale(companyId, s) {
  const { data, error } = await supabase.from('pos_sales').insert({
    company_id: companyId, sale_date: s.date, ts: s.ts, items: s.items, subtotal: s.sub, tax: s.tax, total: s.total,
    method: s.method, received: s.received, change_amount: s.change, status: s.st || 'completed', settled: !!s.settled,
    settled_date: s.settledDate || null, customer_id: s.cid || null, staff: s.staff || '',
  }).select().single();
  return { data, error };
}

export async function dbUpdatePosSale(id, u) {
  const mapped = {};
  if (u.st !== undefined) mapped.status = u.st;
  if (u.settled !== undefined) mapped.settled = u.settled;
  if (u.settledDate !== undefined) mapped.settled_date = u.settledDate;
  return supabase.from('pos_sales').update(mapped).eq('id', id);
}

export async function dbDeletePosSale(id) {
  return supabase.from('pos_sales').delete().eq('id', id);
}

// ===== 買掛金 =====
export async function dbAddPayable(companyId, p) {
  const { data, error } = await supabase.from('payables').insert({
    company_id: companyId, supplier: p.supplier, payable_date: p.date, due_date: p.due, amount: p.amt, tax: p.tax || 0,
    total: p.total, paid: p.paid || 0, status: p.st || 'unpaid', paid_date: p.paidDate || null, description: p.desc || '', account: p.cat || '仕入高',
  }).select().single();
  return { data, error };
}

export async function dbUpdatePayable(id, u) {
  const mapped = {};
  if (u.paid !== undefined) mapped.paid = u.paid;
  if (u.st !== undefined) mapped.status = u.st;
  if (u.paidDate !== undefined) mapped.paid_date = u.paidDate;
  return supabase.from('payables').update(mapped).eq('id', id);
}

export async function dbDeletePayable(id) {
